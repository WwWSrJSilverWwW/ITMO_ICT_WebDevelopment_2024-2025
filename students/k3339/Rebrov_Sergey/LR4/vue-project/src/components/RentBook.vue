  ...
  <v-container>
    ...
    <v-row justify="center" style="margin-top: 50px;">
      <v-col cols="12" md="6">
        <v-card>
          <v-card-title>
            <span class="headline">Выдача книги</span>
          </v-card-title>
          <v-card-text>
            <v-form @submit.prevent="issueBook">
              <v-select
                item-title="fullName"
                item-value="id"
              ...
              <v-select
                ...
                required
              ></v-select>
              <v-btn type="submit" color="primary" block>Выдать книгу</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
      </v-col>
    ...
    async issueBook() {
      try {
        const response = await axios.post(
          "http://127.0.0.1:8000/api/v1/rent_book/",
          {
        ...
        alert("Не удалось выдать книгу. Проверьте данные и попробуйте снова.");
      }
    },
    handleLogout() {
